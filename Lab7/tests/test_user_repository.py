...
async def test_get_user_by_id(user_repository):
    ...
    user_data = UserCreate(
        username="getuser", email="get@example.com", full_name="Get User"
    )
...
async def test_update_user(user_repository):
    ...
    user_data = UserCreate(
        username="updateuser", email="update@example.com", full_name="Update User"
    )
    user = await user_repository.create(user_data)
...
async def test_delete_user(user_repository):
    ...
    user_data = UserCreate(username="deleteuser", email="delete@example.com")
    user = await user_repository.create(user_data)
    user_id = user.id
...
async def test_get_users_with_filter(user_repository):
    ...
    for i in range(5):
        user_data = UserCreate(
            username=f"filteruser{i}", email=f"filter{i}@example.com"
        )
...
async def test_count_users(user_repository):
    ...
    user_data = UserCreate(username="countuser", email="count@example.com")
    await user_repository.create(user_data)
