...
async def test_create_product(product_repository):
    ...
    product = await product_repository.create(
        name="Test Product", price=29.99, stock_quantity=100
    )
...
async def test_get_product_by_id(product_repository):
    ...
    created_product = await product_repository.create(
        name="Get Product", price=19.99, stock_quantity=50
    )
...
async def test_update_product(product_repository):
    ...
    product = await product_repository.create(
        name="Update Product", price=39.99, stock_quantity=75
    )

    updated_product = await product_repository.update(
        product.id, name="Updated Product", price=49.99
    )
...
async def test_update_product_stock(product_repository):
    ...
    product = await product_repository.create(
        name="Stock Product", price=25.00, stock_quantity=100
    )

    updated_product = await product_repository.update(product.id, stock_quantity=50)
...
async def test_delete_product(product_repository):
    ...
    product = await product_repository.create(
        name="Delete Product", price=15.99, stock_quantity=10
    )
...
async def test_list_products(product_repository):
    ...
    for i in range(5):
        await product_repository.create(
            name=f"List Product {i}", price=10.00 + i, stock_quantity=10 * i
        )
...
async def test_list_products_pagination(product_repository):
    ...
    for i in range(10):
        await product_repository.create(
            name=f"Page Product {i}", price=20.00, stock_quantity=10
        )
