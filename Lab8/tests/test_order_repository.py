...
async def test_product(product_repository):
    ...
    product = await product_repository.create(
        name=f"Order Test Product {unique_id}", price=99.99, stock_quantity=100
    )
    yield product
    try:
...
async def test_create_order_single_item(order_repository, test_user, test_product):
    ...
    order = await order_repository.create(user_id=test_user.id, items=items)
...
async def test_create_order_multiple_items(
    order_repository, test_user, product_repository
):
    """Test creating an order with multiple items"""
    product1 = await product_repository.create(
        name=f"Multi Product 1 {uuid.uuid4().hex[:8]}", price=10.00, stock_quantity=50
    )
    product2 = await product_repository.create(
        name=f"Multi Product 2 {uuid.uuid4().hex[:8]}", price=20.00, stock_quantity=50
    )
...
async def test_order_with_invalid_product(order_repository, test_user):
    ...
    with pytest.raises(ValueError, match="Product 99999 not found"):
        await order_repository.create(user_id=test_user.id, items=items)
...
async def test_order_with_invalid_user(order_repository, test_product):
    ...
    with pytest.raises(ValueError):
        await order_repository.create(user_id=99999, items=items)


@pytest.mark.asyncio
async def test_get_order_by_id_with_relationships(
    order_repository, test_user, test_product
):
    """Test retrieving an order by ID includes all relationships"""
    items = [{"product_id": test_product.id, "quantity": 1}]
    created_order = await order_repository.create(user_id=test_user.id, items=items)
...
async def test_update_order_items_add_new(
    ...
    product1 = await product_repository.create(
        name=f"Update Item 1 {uuid.uuid4().hex[:8]}", price=15.00, stock_quantity=100
    )
    product2 = await product_repository.create(
        name=f"Update Item 2 {uuid.uuid4().hex[:8]}", price=25.00, stock_quantity=100
    )
    ...
    updated_order = await order_repository.update(order.id, items=new_items)
...
@pytest.mark.asyncio
async def test_update_order_items_remove(
    order_repository, test_user, product_repository
):
    """Test removing items from an order"""
    product1 = await product_repository.create(
        name=f"Remove Item 1 {uuid.uuid4().hex[:8]}", price=30.00, stock_quantity=100
    )
    product2 = await product_repository.create(
        name=f"Remove Item 2 {uuid.uuid4().hex[:8]}", price=40.00, stock_quantity=100
    )
...
@pytest.mark.asyncio
async def test_update_order_total_recalculated(
    order_repository, test_user, product_repository
):
    """Test that order total is recalculated when items are updated"""
    product = await product_repository.create(
        name=f"Recalc Product {uuid.uuid4().hex[:8]}", price=50.00, stock_quantity=100
    )
...
@pytest.mark.asyncio
async def test_delete_order_cascades_to_items(
    order_repository, test_user, test_product
):
...
@pytest.mark.asyncio
async def test_list_orders_pagination_consistency(
    order_repository, test_user, test_product
):
...
@pytest.mark.asyncio
async def test_order_items_preserve_order(
    order_repository, test_user, product_repository
):
