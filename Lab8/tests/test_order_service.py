...
def order_service_with_mocks(
    mock_order_repository, mock_product_repository, mock_user_repository
):
...
async def test_create_order_user_not_found(
    order_service_with_mocks, mock_user_repository
):
    """Test order creation fails when user not found"""
    mock_user_repository.get_by_id.return_value = None

    order_data = {"user_id": 999, "items": [{"product_id": 1, "quantity": 1}]}
...
async def test_create_order_product_not_found(
    order_service_with_mocks, mock_user_repository, mock_product_repository
):
    ...
    order_data = {"user_id": 1, "items": [{"product_id": 999, "quantity": 1}]}
...
async def test_create_order_insufficient_stock(
    ...
    order_data = {"user_id": 1, "items": [{"product_id": 1, "quantity": 10}]}
...
@pytest.mark.asyncio
async def test_create_order_zero_quantity(
    order_service_with_mocks, mock_user_repository
):
    ...
    order_data = {"user_id": 1, "items": [{"product_id": 1, "quantity": 0}]}
...
async def test_create_order_negative_quantity(
    ...
    order_data = {"user_id": 1, "items": [{"product_id": 1, "quantity": -5}]}
...
@pytest.mark.asyncio
async def test_create_order_multiple_items_stock_validation(
    order_service_with_mocks, mock_user_repository, mock_product_repository
):
...
async def test_update_order_invalid_status(
    order_service_with_mocks, mock_order_repository
):
...
async def test_cancel_order_restores_stock(
    order_service_with_mocks, mock_order_repository, mock_product_repository
):
...
@pytest.mark.asyncio
async def test_get_by_filter_pagination(
    order_service_with_mocks, mock_order_repository
):
...
@pytest.mark.asyncio
async def test_get_by_filter_pagination_pages_differ(
    order_service_with_mocks, mock_order_repository
):
    ...
    mock_order_repository.list.side_effect = [(10, page1_orders), (10, page2_orders)]
...
@pytest.mark.asyncio
async def test_filter_fallback_when_repository_lacks_filters(
    order_service_with_mocks, mock_order_repository
):
